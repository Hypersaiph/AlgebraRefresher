...
class Line(object):
    ...
    def first_nonzero_index(iterable):
        n = len(iterable.coordinates)
        for k in range(n):
            if not MyDecimal(iterable.coordinates[k]).is_near_zero():
                return k
    ...
    def intersection_with(self, ell):
        try:
            ...
            x_numerator = D*float(k1) - B*float(k2)   #Decimal(D)*Decimal(k1) - Decimal(B)*Decimal(k2)
            y_numerator = -C*float(k1) + A*float(k2)  #Decimal(-C)*Decimal(k1) + Decimal(A)*Decimal(k2)
